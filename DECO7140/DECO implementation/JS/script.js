...
]; /* all pictures in candidate list */

let imgElements = document.getElementsByClassName("foodimage");

for (let i = 0; i < imgElements.length; i++) {
    let randomIndex = Math.floor(Math.random() * foodImages.length); /* get a random number */
    imgElements[i].src = foodImages[randomIndex]; /* put it in to the position which should the picture */
    foodImages.splice(randomIndex, 1); /* make share the picture would not be shown repetitive*/
}
/* */
...
function add_to_favorite(foodName) {
    ...
    if (!favorites) {
        favorites = []; /* the initial status, which means no favourite food in it.*/
    } else {
...
} /* make sure user cannot add two identical food into favourite.*/
