...
class Card:
    ...
    def __repr__(self) -> str:
        ...
        class or instance.
        Returns: str: format: f"{self.__class__.__name__}()"
        """
        return f"{self.__class__.__name__}()"
    ...
    def player_apply_card(self, card_name=str, target_id: int | None = None) -> bool:
        ...
        for cards in self._player.get_hand():
            if card_name == cards.get_name():
                if cards.requires_target() == True:  # choose those cards which
                    # requires a target.
                    if target_id == None:
                        return False
                    list_for_monster_id = []
                    for monster in self._monster_list:
                        list_for_monster_id.append(monster.get_id())  # obtain all
                        # monsters' id in this encounter
                    if target_id not in list_for_monster_id:
                        return False
        play_card_result = self._player.play_card(card_name)  # check if it is
        # possible for player to play the card he want.
                    ...
                    try:
                        ...
                        if monster.get_weak() > 0:
                            if self._player.get_vulnerable() > 0:
                                ...
                                self._player.add_weak(monster.action()["weak"])
                                # Cultist's attack some time would apply weak to
                                # player.
                            else:
                                self._player.reduce_hp(
                                    (monster.action()["damage"]) * 0.75
...
def main():
    ...
    if ironclad == IronClad().get_name():
        player = IronClad()
    elif player_user_select.capitalize() == Silent().get_name():  # tranfer the first
        # letter to capital.
        player = Silent()
    elif player_user_select.capitalize() == Watcher().get_name():
        player = Watcher()
    game_file_player_select = input("Enter a game file: ")  # Obtain what game user
    # want to play.
    read_game_file(game_file_player_select)
    encounter = Encounter(player, read_game_file(game_file_player_select)[0])
    display_encounter(encounter)
    # read_game_file(game_file_player_select) would return a list of instances of
    # Monster().
    for monster in encounter._monster_list:  # for the number of monster(s) it would
        # vary the encounter time. The following code is for a single encounter.
        display_encounter(encounter)
        while monster.is_defeated() == False:
            players_move = input("Enter a move: ")  # if monster doesn't be defeat,
            # keep the loop
            if players_move == "end turn":
                encounter.end_player_turn()
                encounter.enemy_turn()
                if player.is_defeated() == True:  # if you are defeated in enermy's
                    # turn print losing message and end the loop.
                ...
                print("" + "\n" + Eruption().get_description() + "\n" + "")
            elif players_move == "describe Vigilance":
                print("" + "\n" + Vigilance().get_description() + "\n" + "")
            elif players_move[:5] == "play ":  # determine if player's command is want
                # to play somecards.
                if players_move == "play Defend":
                    encounter.player_apply_card("Defend")
                    if encounter.player_apply_card("Defend") == False:  # if player
                        # cannot apply the card for certain reason, print fail message.
                        print("" + "\n" + CARD_FAILURE_MESSAGE + "\n" + "")
                elif players_move == "play Survivor":
                    encounter.player_apply_card("Survivor")
                    if encounter.player_apply_card("Survivor") == False:
                        print("" + "\n" + CARD_FAILURE_MESSAGE + "\n" + "")
                elif players_move == "play Vigilance":
                    encounter.player_apply_card("Vigilance")
                    if encounter.player_apply_card("Vigilance") == False:
                        print("" + "\n" + CARD_FAILURE_MESSAGE + "\n" + "")
                elif players_move[:12] == "play Strike ":  # ensure the player want
                    # to apply the card "Strike"
                    target_id = int(players_move[12:])  # gain the target number
                    # from player's command. from position 12 in "play Strike
                    # {target_id}", it would be a number. define it as target id
                    # player want to attack.
                    encounter.player_apply_card("Strike", target_id)
                    if encounter.player_apply_card("Strike", target_id) == False:
                        print("" + "\n" + CARD_FAILURE_MESSAGE + "\n" + "")
                elif players_move[:10] == "play Bash ":
                    target_id = int(players_move[10:])
                    encounter.player_apply_card("Bash", target_id)
                    if encounter.player_apply_card("Bash", target_id):
                        print("" + "\n" + CARD_FAILURE_MESSAGE + "\n" + "")
                elif players_move[:16] == "play Neutralize ":
                    target_id = int(players_move[16:])
                    encounter.player_apply_card("Neutralize", target_id)
                    if encounter.player_apply_card("Neutralize", target_id) == False:
                        print("" + "\n" + CARD_FAILURE_MESSAGE + "\n" + "")
                elif players_move[:14] == "play Eruption ":
                    target_id = int(players_move[14:])
                    encounter.player_apply_card("Eruption", target_id)
                    if encounter.player_apply_card("Eruption", target_id) == False:
                        print("" + "\n" + CARD_FAILURE_MESSAGE + "\n" + "")
                display_encounter(encounter)
        else:  # if the monster in this encounter is defeated, end the loop.
            print(ENCOUNTER_WIN_MESSAGE)
            break
    if encounter._monster_list == []:  # if all monster is defeated, print win message
        # and end the loop
