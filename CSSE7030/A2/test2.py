    ...
    def player_apply_card(self, card_name=str, target_id: int | None = None) -> bool:
        ...
        for cards in self._player.get_hand():
            if card_name == cards.get_name():
                if (cards.requires_target() == True):  # choose those cards which requires a target.
                    if target_id == None:
                        return False
                    list_for_monster_id = []
                    for monster in self._monster_list:
                        list_for_monster_id.append(monster.get_id())  # obtain all monsters' id in this encounter
                    if target_id not in list_for_monster_id:
                        return False
        play_card_result = self._player.play_card(card_name) # check if it is possible for player to play the card he want.
                    ...
                    if self._player.get_strength() > 0:
                        if monster.get_vulnerable() > 0:
                            if self._player.get_weak() > 0:
                                monster.reduce_hp(int(6 + self._player.get_strength()) * 1.5 * 0.75)
                            else:
                                monster.reduce_hp(int(6 + self._player.get_strength()) * 1.5)
                        else:
            ...
            for monster in self._monster_list:
                if monster.get_id() == target_id:
                    if self._player.get_strength() > 0:
                        if monster.get_vulnerable() > 0:
                            if self._player.get_weak() > 0:
                                monster.reduce_hp(int(7 + self._player.get_strength()) * 1.5 * 0.75)
                            else:
                                monster.reduce_hp(int(7 + self._player.get_strength()) * 1.5)
                        else:
            ...
            for monster in self._monster_list:
                ...
                if monster.get_id() == target_id:
                    if self._player.get_strength() > 0:
                        if monster.get_vulnerable() > 0:
                            if self._player.get_weak() > 0:
                                monster.reduce_hp(int(3 + self._player.get_strength()) * 1.5 * 0.75)
                            else:
                                monster.reduce_hp(int(3 + self._player.get_strength()) * 1.5)
                        else:
            ...
            for monster in self._monster_list:
                if monster.get_id() == target_id:
                    if self._player.get_strength() > 0:
                        if monster.get_vulnerable() > 0:
                            if self._player.get_weak() > 0:
                                monster.reduce_hp(int(9 + self._player.get_strength()) * 1.5 * 0.75)
                            else:
                                monster.reduce_hp(int(9 + self._player.get_strength()) * 1.5)
                        else:
...
def main():
    ...
    ironclad = player_user_select[:1].upper() + player_user_select[1:4] + player_user_select[4:5].upper() + player_user_select[5:]
    # transfer ironclad tp IronClad.
    if ironclad == IronClad().get_name():
        player = IronClad()
    elif player_user_select.capitalize() == Silent().get_name(): # tranfer the first letter to capital.
        player = Silent()
    elif player_user_select.capitalize() == Watcher().get_name():
        player = Watcher()
    game_file_player_select = input("Enter a game file: ") # Obtain what game user want to play.
    read_game_file(game_file_player_select)
    
    encounter = Encounter(player, read_game_file(game_file_player_select)[0]) # read_game_file(game_file_player_select) would return a list of instances of Monster().
    for monster in encounter._monster_list: # for the number of monster(s) it would vary the encounter time. The following code is for a single encounter.
        print("New encounter!")
        print("")
        print("MONSTERS")
        print("-" * 15)
        ...
        while monster.is_defeated() == False:  
            players_move = input("Enter a move: ") # if monster doesn't be defeat, keep the loop
            if players_move == "end turn":
                encounter.end_player_turn()
                encounter.enemy_turn()
                if player.is_defeated() == True: # if you are defeated in enermy's turn print losing message and end the loop.
                ...
                if encounter.player_apply_card("Defend") == True: # check if player can appliy the card successful.
                    print("MONSTERS")
                    print("-" * 15)
                    print("Monster", monster.get_id())
                    print(monster.__str__())
                ...
                if encounter.player_apply_card("Vigilance") == True:
                    ...
                    print("Card application failed.")
                    print("")
            elif players_move[:12] == "play Strike ": # ensure the player wan to apply the card "Strike"
                target_id = int(players_move[12:]) # gain the target number from player's command. from position 12 in "play Strike {target_id}", it would be a number. define it as target id player want to attack.
            ...
            break
    if encounter._monster_list == []: # if all monster is defeated, print win message and end the loop
